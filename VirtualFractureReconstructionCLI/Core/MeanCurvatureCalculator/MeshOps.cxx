...
void ComputeCurvatureData( MeshDataCustom* meshdata )
{
  ...
  meshdata->dkmag.resize(numverts);
  //std::cout << "After allocation, nx = " << meshdata->nx.size() << std::endl;
...
void SmoothCurvature( MeshDataCustom* meshdata )
{
  ...
  std::cout<<"Smoothing curvature..."<<std::endl;
...
void ComputeCurvature( MeshDataCustom* meshdata )
{
  if (meshdata == NULL ||
      ...
      meshdata->nz.size() == 0)
    {
    std::cerr << "Compute Curvature: no normals on mesh data"<<std::endl;
    return;
    }
...
// return a vector containing all indices of the initalized contour
  ...
  if( numPts < 3 )
    {
    std::cerr<<"Must have at least 3 pts to define closed contour! "<<std::endl;
    return vector<int>(0);
    }
  ...
  if ( meshdata->adjimm.size() == 0)
    {
    std::cerr << "InitPath: No mean adjacency computed on mesh"<<std::endl;
    return C;
    }
  double thispt[3];
  double thatpt[3];
...
void ComputeNormals( MeshDataCustom* meshdata )
{
  ...
  if( bTextInputNormals )
    ...
    if( ! bTextInputNormals ) {
      ...
      {
      std::cerr << "Normals not computed for mesh data, numverts = " << numverts << ", normal array sizes were allocated to be x = " << meshdata->nx.size() << ", y = " << meshdata->ny.size() << ", z = " << meshdata->nz.size() << std::endl;
      }
    }
}
...
void ComputeAdjacency( MeshDataCustom* meshdata )
{
  ...
  for( int i = 0; i < numverts; i++ ) {
    meshdata->adjimm[i].myNeighbs    = vector<int>(1);
    meshdata->adjimm[i].myNeighbs[0] = i;
    meshdata->adjimm[i].myIdx = i;
  }
...
//    int vert2 = pts[2];
  ...
  MeshDataCustom* tempdata = new MeshDataCustom();
