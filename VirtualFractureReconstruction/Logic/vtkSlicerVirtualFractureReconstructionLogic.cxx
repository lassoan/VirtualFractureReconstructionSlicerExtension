...
//---------------------------------------------------------------------------
void vtkSlicerVirtualFractureReconstructionLogic::OnMRMLSceneNodeAdded(vtkMRMLNode* vtkNotUsed(node))
{
}

//---------------------------------------------------------------------------
void vtkSlicerVirtualFractureReconstructionLogic::OnMRMLSceneNodeRemoved(vtkMRMLNode* vtkNotUsed(node))
{
}
...
{
  //vtkDebugMacro("qMRMLLabelComboBox::colorFromIndex - index:" << index);
...
bool vtkSlicerVirtualFractureReconstructionLogic::CreateModel(vtkMRMLVirtualFractureReconstructionNode* reconstructionNode,
  vtkMRMLLabelMapVolumeNode* volumeNode, vtkMRMLModelHierarchyNode* hierarchyNode, bool reference)
{
  ...
  if (reconstructionNode->GetCurrentFragmentModelHierarchyNodeID() && !hierarchyNode)
  {
    modelHierarchyNode = vtkMRMLModelHierarchyNode::SafeDownCast(
      this->GetMRMLScene()->GetNodeByID(reconstructionNode->GetCurrentFragmentModelHierarchyNodeID()));
    modelMakerNode->SetDescription("ModelMakerNode");
  }
  else if (hierarchyNode)
  {
    vtkDebugMacro("Yes we do have a hierarchy node");
    modelHierarchyNode = hierarchyNode;
    vtkDebugMacro("Model hierarchy node " << modelHierarchyNode->GetID() << "will be used");
  }
  else
  {
    vtkDebugMacro("New model hierarchy node will be created");
  ...
  vtkDebugMacro("Using model hierarchy node" << modelHierarchyNode->GetID() << " as model scene id.");
  ...
  vtkDebugMacro("model maker input set");
  ...
  vtkDebugMacro("Model hierarchy node with ID " << hierarchy->GetID() << "  was created.");
  ...
  vtkDebugMacro("ModelDisplayNode: " << modelNode->GetModelDisplayNode() << "with color index " << labelVal << " created ");
...
::StartRegistrationStep1(vtkMRMLVirtualFractureReconstructionNode* reconstructionNode)
{
  ...
  vtkDebugMacro("Creating FractureReconstructionCLI node for logic " << this->Internal->CLIReconstructorLogic);
  reconstructionNodeCLI = this->Internal->CLIReconstructorLogic->CreateNodeInScene();
  assert(reconstructionNodeCLI.GetPointer() != 0);
  vtkDebugMacro("Setting CLINodeID " << reconstructionNodeCLI->GetID() << " for reconstruction node with address" << reconstructionNode);
  ...
  vtkDebugMacro("Parameters for registration set.Starting registration");
...
{
  vtkSmartPointer<vtkMRMLCommandLineModuleNode> reconstructionNodeCLI;
  vtkDebugMacro("Creating FractureReconstructionCLI node for fine tuning " << this->Internal->CLIReconstructorLogic);
  reconstructionNodeCLI = this->Internal->CLIReconstructorLogic->CreateNodeInScene();
  assert(reconstructionNodeCLI.GetPointer() != 0);
  ...
  vtkDebugMacro("CS-InputTransform: " << this->GetCLIReconstructionPropertyNode()->GetCommaSeparatedInputTransformIDs());
  vtkDebugMacro("CS-OutputTransform: " << this->GetCLIReconstructionPropertyNode()->GetCommaSeparatedTransformIDs());
...
//---------------------------------------------------------------------------
void vtkSlicerVirtualFractureReconstructionLogic::CorrectPose(std::string incorrectModelID,
  std::string correctNodeID, std::string additionalTransformNodeID, bool invert)
{
...
//---------------------------------------------------------------------------
std::vector<std::string > vtkSlicerVirtualFractureReconstructionLogic::CreateScalarVolumeNodesFromMultiLabelNode(
  vtkSmartPointer<vtkMRMLScalarVolumeNode> node, std::map<unsigned char, unsigned int> labelColors, 
  vtkSmartPointer<vtkMRMLScalarVolumeNode> referenceNode)
{
  ...
  for (it = volumeOrderedImages.begin(); it != volumeOrderedImages.end(); ++it)
  {
    ...
    vtkDebugMacro("ADDING LABEL WITH " << (*it).first << " VOXELS");
    counter++;
  }
  //imageMap.clear();
...
void vtkSlicerVirtualFractureReconstructionLogic::UpdateStep2Parameters(vtkSmartPointer<vtkMRMLCommandLineModuleNode> reconstructionNodeCLI)
{
  ...
  vtkDebugMacro("SLICER TEMP LOGIG" << reconstructionNodeCLI->GetParameterAsString("slicerTempPath").c_str());
...
//---------------------------------------------------------------------------
void vtkSlicerVirtualFractureReconstructionLogic::UpdateStep1Parameters(
  vtkSmartPointer<vtkMRMLCommandLineModuleNode> reconstructionNodeCLI, vtkMRMLVirtualFractureReconstructionNode* reconstructionNode)
{
  const char* tempPath = qSlicerCoreApplication::application()->temporaryPath().toUtf8().constData();
  reconstructionNodeCLI->SetParameterAsString("slicerTempPath", tempPath);
  vtkDebugMacro("SLICER TEMP LOGIG" << reconstructionNodeCLI->GetParameterAsString("slicerTempPath").c_str());
